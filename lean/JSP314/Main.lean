...
* `badNonSingleton_interval_bound`: the **hard analytic core** of Ta26c
  ...
  left as a single `sorry`, clearly marked.
...
/-- **Hard analytic core of Ta26c (single `sorry`).**
...
No `sorry` is spread elsewhere: the entire remaining mathematical content of the
conjecture is isolated in this one lemma. -/
